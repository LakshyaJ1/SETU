...
class SetuRepository(private val context: Context) {
    ...
    fun updateSettings(requested: AppSettings) {
        ...
        require(requested.walkingStepLengthMeters.isFinite() && requested.walkingStepLengthMeters in 0.3..1.2)
        if (recording.value && (requested.vehicle != previous.vehicle || requested.walkingStepLengthMeters != previous.walkingStepLengthMeters)) {
            appendRecord(JSONObject().put("type", "collection_change").put("tNs", SystemClock.elapsedRealtimeNanos())
                .put("vehicle", requested.vehicle).put("walkingStepLengthMeters", requested.walkingStepLengthMeters))
        }
        ...
        preferences.edit().putString("theme", updated.theme).putString("units", updated.units)
            ...
            .putFloat("walkingStepLengthMeters", updated.walkingStepLengthMeters.toFloat())
    ...
    private fun readSettings() = AppSettings(
        ...
        preferences.getFloat("walkingStepLengthMeters", 0.70f).toString().toDouble().takeIf { it.isFinite() && it in 0.3..1.2 } ?: 0.70,
    )

    @Synchronized
    fun beginRecording(name: String, fixedMount: Boolean = false) {
        ...
        try {
            ...
            .put("collection", CollectionMetadata.create(context, recordingId, settings.value, fixedMount))
        ...
        lateinit var active: LogRecorder
        active = LogRecorder(output, { count, dropped ->
            ...
            storageScope.launch {
                synchronized(this@SetuRepository) {
                    if (recorder === active) {
    ...
    private fun deriveTrackPose(): JSONObject? {
    ...
    private fun refreshModelSession() {
        ...
        if (configuration.vehicle == "Walking") {
            mutableModelInference.value = ModelInferenceState("Walking uses detected steps", "The car speed model is disabled for walking. No model speed is used or shared.")
            return
        }
        ...
        if (onDevice && !OnDeviceModelProvider.supportsVehicle(context, configuration.vehicle)) {
            mutableModelInference.value = ModelInferenceState("Collecting for ${configuration.vehicle}",
                "GPS and IMU recording work. The bundled speed model does not cover this activity; it is not running. Export recordings for a separately validated model.")
            return
        }
        ...
        try {
            ...
            hub.modelSession = LiveModelSession(provider, configuration.vehicle,
                ...
                { state -> synchronized(this) {
                    ...
                    val logResult: (Boolean) -> Unit = { applied -> synchronized(this) {
                        if (generation == modelGeneration && recorder === activeRecorder && recording.value) {
                    ...
                    if (onDevice && measurement != null && measurement.validity > 0.0) {
                        hub.applyModelSpeed(measurement.timestampNs, measurement.speedMps, measurement.sigmaMps, logResult)
    ...
    private fun appendRecord(record: JSONObject) {
        ...
        if (record.optString("type") in listOf("pose", "native_pose")) deriveTrackPose()?.let(active::document)
    }

    @Synchronized
    fun finishRecording(): Trip? {
        if (!mutableRecording.value) return null
        hub.record = null
        hub.sensorSink = null
        val active = recorder
        recorder = null
